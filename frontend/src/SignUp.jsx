...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .grid-bg { position: fixed; inset: 0; background-image: radial-gradient(circle, rgba(255,255,255,0.055) 1px, transparent 1px); background-size: 38px 38px; pointer-events: none; z-index: 0; }
  ...
  .su-page { position: relative; z-index: 1; width: 100%; max-width: 460px; animation: fadeUp 0.45s ease both; }
  @keyframes fadeUp { from{opacity:0;transform:translateY(22px);} to{opacity:1;transform:translateY(0);} }

  .brand-bar { display: flex; align-items: center; justify-content: center; gap: 10px; margin-bottom: 32px; }
  .brand-gem { width: 34px; height: 34px; background: linear-gradient(135deg, var(--accent2), var(--accent)); border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 16px; box-shadow: 0 0 20px rgba(167,139,250,0.5); }
  .brand-name { font-family: 'Playfair Display', serif; font-size: 20px; background: linear-gradient(90deg, var(--accent2), var(--accent)); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }

  .su-card { background: var(--card); border: 1px solid var(--card-border); border-radius: var(--radius); overflow: hidden; position: relative; }
  .su-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 2px; background: linear-gradient(90deg, transparent, var(--accent2), var(--accent), transparent); }

  .su-hero { padding: 34px 32px 24px; text-align: center; border-bottom: 1px solid var(--card-border); position: relative; overflow: hidden; }
  .su-hero::after { content: ''; position: absolute; left: -50px; top: -50px; width: 200px; height: 200px; background: radial-gradient(circle, rgba(167,139,250,0.07) 0%, transparent 70%); border-radius: 50%; pointer-events: none; }

  .su-avatar { width: 64px; height: 64px; border-radius: 50%; margin: 0 auto 16px; background: linear-gradient(var(--card), var(--card)) padding-box, linear-gradient(135deg, var(--accent2), var(--accent)) border-box; border: 2px solid transparent; display: flex; align-items: center; justify-content: center; font-size: 26px; box-shadow: 0 0 22px rgba(167,139,250,0.3); position: relative; z-index: 1; }
  .su-title { font-family: 'Playfair Display', serif; font-size: 24px; font-weight: 700; margin-bottom: 6px; background: linear-gradient(90deg, var(--accent2), var(--accent)); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; position: relative; z-index: 1; }
  .su-sub { font-size: 13.5px; color: var(--muted); position: relative; z-index: 1; }

  .su-form-body { padding: 26px 32px 28px; display: flex; flex-direction: column; gap: 18px; }
  ...
  /* inputs without left icon (username) */
  ...
  .field-input.no-left-icon { padding-left: 15px; }
  .field-input::placeholder { color: var(--muted); }
  .field-input:focus { border-color: var(--accent2); box-shadow: 0 0 0 3px rgba(167,139,250,0.13); background: rgba(167,139,250,0.04); }
  ...
  .error-msg { font-size: 12px; color: var(--danger); display: flex; align-items: center; gap: 5px; }
  ...
  /* Password strength */
  .strength-section { display: flex; flex-direction: column; gap: 6px; animation: fadeUp 0.25s ease both; }
  .strength-bar-row { display: flex; gap: 4px; margin-bottom: 2px; }
  .strength-seg { flex: 1; height: 4px; border-radius: 4px; background: var(--card-border); transition: background 0.3s; }
  ...
  .strength-label { font-size: 11px; font-weight: 600; letter-spacing: 0.06em; text-transform: uppercase; margin-bottom: 4px; }
  .strength-label.weak   { color: var(--danger); }
  .strength-label.medium { color: var(--gold); }
  .strength-label.strong { color: var(--accent3); }
  .validation-list { display: flex; flex-direction: column; gap: 4px; }
  .val-item { display: flex; align-items: center; gap: 8px; font-size: 12.5px; transition: color 0.25s; }
  .val-item.pass { color: var(--accent3); }
  .val-item.fail { color: var(--muted); }
  .val-dot { width: 16px; height: 16px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 9px; flex-shrink: 0; transition: background 0.25s; }
  ...
  .submit-btn { width: 100%; padding: 13px; border-radius: 50px; border: none; background: linear-gradient(135deg, var(--accent2), var(--accent)); color: white; font-family: 'Outfit', sans-serif; font-size: 14.5px; font-weight: 700; cursor: pointer; letter-spacing: 0.03em; transition: transform 0.18s, box-shadow 0.18s, opacity 0.18s; box-shadow: 0 4px 22px rgba(167,139,250,0.38); display: flex; align-items: center; justify-content: center; gap: 8px; margin-top: 4px; }
  ...
  .su-footer { padding: 0 32px 26px; text-align: center; font-size: 13px; color: var(--muted); }
  .su-footer a { color: var(--accent2); font-weight: 600; text-decoration: none; transition: color 0.15s; }
  .su-footer a:hover { color: var(--accent); }

  .terms-note { text-align: center; font-size: 11.5px; color: var(--muted); line-height: 1.6; margin-top: -4px; }
        ...
        <div className="su-page">
          ...
          <div className="su-card">
            ...
            <form onSubmit={handleSubmit} className="su-form-body">
              {errorMsg && <div className="api-error">⚠️ {errorMsg}</div>}
              ...
              <div className="field-group">
                ...
                <div className="field-wrap">
                  <input
                    className="field-input no-left-icon"
                    type="text"
                    value={name}
                ...
                {password.length > 0 && strength && (
                  <div className="strength-section">
                    <div className="strength-bar-row">
                      {[0, 1, 2].map(i => (
                        <div key={i} className={`strength-seg ${i < strength.segs ? `fill-${strength.cls}` : ''}`} />
                      ))}
                    </div>
                    {strength.label && (
